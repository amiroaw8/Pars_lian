...
@section('title', 'ویرایش هزینه - پارس لیان')

@section('content')
<div class="max-w-4xl mx-auto space-y-12 pb-12">
    <x-page-header 
        title="ویرایش هزینه" 
        subtitle="بروزرسانی اطلاعات هزینه ثبت شده برای اطمینان از صحت گزارشات مالی."
        badge="Update Expense"
        badgeIcon="ti-edit"
        headerIcon="ti-cash-banknote"
    />

    <form action="{{ route('automation.accounting.expenses.update', $expense) }}" method="POST" class="animate-slide-up">
        @csrf
        @method('PUT')
        <x-enhanced-card title="ویرایش جزئیات" icon="ti-info-circle">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 p-4">
                <div class="form-group-modern group">
                    ...
                    <input type="text" name="title" id="title" class="form-control-modern @error('title') border-rose-500 @enderror" value="{{ old('title', $expense->title) }}" placeholder="مثلاً: اجاره دفتر مهر ماه" required>
                ...
                <div class="form-group-modern group">
                    ...
                    <input type="number" name="amount" id="amount" class="form-control-modern @error('amount') border-rose-500 @enderror" value="{{ old('amount', (int)$expense->amount) }}" placeholder="مثلاً: 5000000" min="0" required>
                ...
                <div class="form-group-modern group">
                    ...
                    <select name="category" id="category" class="form-control-modern @error('category') border-rose-500 @enderror" required>
                        ...
                        @foreach(['اجاره', 'قبوض', 'خرید قطعات', 'حقوق', 'سایر'] as $cat)
                ...
                <div class="form-group-modern group">
                    ...
                    <input type="text" name="expense_date" id="expense_date" class="form-control-modern @error('expense_date') border-rose-500 @enderror" value="{{ old('expense_date', \Morilog\Jalali\Jalalian::fromCarbon($expense->expense_date)->format('Y/m/d')) }}" placeholder="۱۴۰۴/۰۱/۰۱" dir="ltr" required>
                ...
                <div class="form-group-modern group md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" class="form-control-modern @error('description') border-rose-500 @enderror" placeholder="توضیحات بیشتر در مورد این هزینه...">{{ old('description', $expense->description) }}</textarea>
