...
<div class="page-header animate-slide-up">
    <div>
        ...
        <div class="breadcrumb text-secondary-600">
            ...
            <span>{{ \Illuminate\Support\Facades\Auth::user()->getRoleDisplayName() }}</span>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 animate-slide-up" style="animation-delay: 0.15s;">
    <div class="stat-card-admin bg-gradient-to-br from-primary-600 to-primary-700 p-5 rounded-2xl shadow-lg text-white group overflow-hidden relative">
        ...
        <div class="relative z-10">
            <div class="text-xs font-medium text-primary-100 mb-1">نرخ بازگشت مشتری</div>
            <div class="flex items-end gap-2">
                <div class="text-3xl font-black">{{ $stats['customer_retention'] ?? 0 }}%</div>
                <div class="text-[10px] text-primary-200 mb-1.5">BI Insights</div>
            </div>
        </div>
    </div>

    <div class="stat-card-admin bg-white p-5 rounded-2xl border border-slate-100 shadow-sm group">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center group-hover:bg-amber-600 group-hover:text-white transition-all">
                <i class="ti ti-alert-triangle text-xl"></i>
            </div>
            <div>
                <div class="text-xs font-medium text-slate-500 mb-0.5">موجودی رو به اتمام</div>
                <div class="text-xl font-black text-slate-800">{{ $stats['low_stock_items'] ?? 0 }}</div>
            </div>
        </div>
    </div>

    <div class="stat-card-admin bg-white p-5 rounded-2xl border border-slate-100 shadow-sm group col-span-1 lg:col-span-2">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <div class="text-xs font-medium text-slate-500 mb-0.5">درآمد تخمینی ماه جاری</div>
                    <div class="text-xl font-black text-slate-800">
                        {{ number_format($currentMonthRevenue ?? 0) }} <span class="text-[10px] font-medium">تومان</span>
                    </div>
                </div>
            </div>
            <a href="{{ route('automation.accounting.index') }}" class="text-xs font-bold text-primary-600 hover:text-primary-700 bg-primary-50 px-3 py-1.5 rounded-lg transition-colors">
                جزئیات مالی
            </a>
        </div>
    </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 animate-slide-up" style="animation-delay: 0.1s;">
    <div class="stat-card-admin bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group min-w-0">
        <div class="flex items-center gap-3 sm:gap-4 min-w-0">
            <div class="w-11 h-11 sm:w-12 sm:h-12 shrink-0 rounded-xl bg-primary-50 text-primary-600 flex items-center justify-center group-hover:bg-primary-600 group-hover:text-white transition-all">
    ...
    <div class="stat-card-admin bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group min-w-0">
        <div class="flex items-center gap-3 sm:gap-4 min-w-0">
            <div class="w-11 h-11 sm:w-12 sm:h-12 shrink-0 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center group-hover:bg-emerald-600 group-hover:text-white transition-all">
    ...
    <div class="stat-card-admin bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group min-w-0">
        <div class="flex items-center gap-3 sm:gap-4 min-w-0">
            <div class="w-11 h-11 sm:w-12 sm:h-12 shrink-0 rounded-xl bg-sky-50 text-sky-600 flex items-center justify-center group-hover:bg-sky-600 group-hover:text-white transition-all">
    ...
    <div class="stat-card-admin bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group min-w-0">
        <div class="flex items-center gap-3 sm:gap-4 min-w-0">
            <div class="w-11 h-11 sm:w-12 sm:h-12 shrink-0 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition-all">
    ...
    <div class="stat-card-admin bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group min-w-0">
        <div class="flex items-center gap-3 sm:gap-4 min-w-0">
            <div class="w-11 h-11 sm:w-12 sm:h-12 shrink-0 rounded-xl bg-violet-50 text-violet-600 flex items-center justify-center group-hover:bg-violet-600 group-hover:text-white transition-all">
    ...
    <div class="stat-card-admin bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group min-w-0">
        <div class="flex items-center gap-3 sm:gap-4 min-w-0">
            <div class="w-11 h-11 sm:w-12 sm:h-12 shrink-0 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center group-hover:bg-amber-600 group-hover:text-white transition-all">
    ...
    <div class="stat-card-admin bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group flex flex-col gap-3 min-w-0">
        <div class="flex items-center gap-3 min-w-0">
            <div class="w-11 h-11 sm:w-12 sm:h-12 shrink-0 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center group-hover:bg-purple-600 group-hover:text-white transition-all">
        ...
        <a href="{{ route('admin.files.index') }}" class="flex items-center justify-center gap-1.5 w-full text-xs font-bold text-purple-600 hover:text-purple-700 bg-purple-50 hover:bg-purple-100 px-3 py-2 rounded-lg transition-colors">
            <i class="ti ti-folder-open text-sm shrink-0"></i>
            <span class="truncate">مدیریت فایل‌ها</span>
        </a>
    </div>
</div>
    ...
    <x-enhanced-card title="وضعیت فعلی سفارشات" icon="ti-adjustments-horizontal">
        <div class="space-y-6">
            @php
                ...
                $totalStatusOrders = max(1, array_sum(array_column($statusBreakdown, 'count')));
            @endphp

            <div class="relative pt-1">
                @forelse($statusBreakdown as $data)
                    @php $percent = ($data['count'] / $totalStatusOrders) * 100; @endphp
                    <div class="mb-4">
                ...
                @empty
                    <p class="text-sm text-slate-400 text-center py-6">سفارش تعمیری ثبت نشده است.</p>
                @endforelse
            </div>
        </div>
    </x-enhanced-card>
            ...
            @forelse($advancedStats['top_selling_products'] ?? [] as $product)
                <div class="flex items-center gap-4 p-3 rounded-xl bg-slate-50/50 border border-slate-100">
                    ...
                    <div class="text-xs font-black text-emerald-600">
                        {{ number_format($product['total_revenue']) }} <span class="text-[9px]">تومان</span>
                    </div>
                </div>
            @empty
